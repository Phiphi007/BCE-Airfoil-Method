...
def get_B_matrix(n, num_points=2000):
    t = np.linspace(0, 1, num_points)
    B = np.zeros((num_points, n + 1))
    for i in range(n + 1):
        B[:, i] = math.comb(n, i) * (t**i) * ((1 - t)**(n - i))
    return B
...
def get_bce_airfoil(p, X_eval_targets, mode='Free', Zt_ref=None, Yc_ref=None):
    """
    【核心革命】：直接在底层解析几何生成时，实施物理锁死！
    绝不去反解那些发疯的控制点，而是直接覆盖空间几何变量。
    """
    ...
    # ===============================================
    # 显式物理特征锁死 (Explicit Geometric Override)
    # ===============================================
    ...
    chord = np.maximum(np.sqrt((X_TE_raw - X_LE_raw)**2 + (Y_TE_raw - Y_LE_raw)**2), 1e-8)
    ...
    def transform(x, y):
        ...
        return (xs * np.cos(theta) - ys * np.sin(theta)) / chord, (xs * np.sin(theta) + y * np.cos(theta)) / chord

    X_up_norm, Y_up_norm = transform(X_raw_eval, Y_up_raw)
    X_dn_norm, Y_dn_norm = transform(X_raw_eval, Y_dn_raw)
    X_cam_norm, Y_cam_norm = transform(xc_raw, yc_raw)
...
Y_up_opt, Y_dn_opt, Y_bce_cam_opt = get_bce_airfoil(p_opt, X_target)
max_err = np.max(np.abs(np.concatenate([Y_up_opt-Y_up_target, Y_dn_opt-Y_dn_target])))
print(f"[+] 初次拟合成功！最大归一化残差: {max_err:.6f}")
print("\n[*] 即将开启交互式气动设计面板 ...")
...
class InteractiveBCE:
    def __init__(self, p_base):
        ...
        plt.subplots_adjust(left=0.22, bottom=0.05, top=0.88, right=0.98)
        ...
        self.fig.suptitle(f"{title_str}\nInteractive Live Editor (Max Error: {max_err:.5f})", fontsize=16, fontweight='bold', y=0.97)

        B_matrix = get_B_matrix(7, 2000)
        self.B_h_pinv = np.linalg.pinv(B_matrix[:, 0:7]) 
        ...
        # UI 控制区
        ax_radio = plt.axes([0.02, 0.65, 0.16, 0.15], facecolor='none') 
        ax_radio.axis('off') 
        # 最安全兼容版本的 RadioButtons
        self.radio = RadioButtons(ax_radio, ('Free', 'Lock Zt', 'Lock Yc'))
        
        self.radio.on_clicked(self.on_mode_change)
            
        ax_reset = plt.axes([0.02, 0.55, 0.15, 0.05])
        self.btn_reset = Button(ax_reset, 'Reset', color='#ff6b6b', hovercolor='#ff4757')
    ...
    def compute_reference_fields(self):
        ...
        self.t_eval = np.linspace(0, 1, 2000)
        h_val = np.maximum(np.interp(self.t_eval, h_curve[0], h_curve[1]), 1e-5)
        alpha_val = np.radians(np.interp(self.t_eval, a_curve[0], a_curve[1]))
        beta_val = np.radians(np.maximum(np.interp(self.t_eval, b_curve[0], b_curve[1]), 0.1))
    ...
    def update_title(self):
        status = ""
        if self.mode == 'Lock Zt': status = " [Zt Locked: Airfoil Thickness is frozen]"
        elif self.mode == 'Lock Yc': status = " [Yc Locked: Camber Bone is frozen]"
        self.fig.suptitle(f"{title_str}\nInteractive Live Editor{status}", fontsize=16, fontweight='bold', y=0.97)
    ...
    def init_plots(self):
        ...
        Y_up, Y_dn, Y_bce_cam = get_bce_airfoil(self.p, X_target)
        self.line_up, = self.ax1.plot(X_target, Y_up, 'b-', linewidth=2, label='Upper')
        self.line_dn, = self.ax1.plot(X_target, Y_dn, 'g-', linewidth=2, label='Lower')
        self.line_bce_cam, = self.ax1.plot(X_target, Y_bce_cam, color='orange', linestyle='--', linewidth=1.5, label='BCE Theory Bone')
        self.ax1.axis('equal')
        ...
        self.ax2.set_title("2. Thickness Control h(x)", fontsize=11)
        ...
        self.ax3.set_title("3. Camber Control $\\alpha(x)$", fontsize=11)
        ...
        self.ax4.set_title("4. Fullness Control $\\beta(x)$", fontsize=11)
    ...
    def on_press(self, event):
        ...
        for ax, pts_artist, offset in zip(
                [self.ax2, self.ax3, self.ax4], [self.pts_h, self.pts_a, self.pts_b], [0, 7, 14]):
            if event.inaxes == ax:
                ...
                if cont:
                    ...
                    if self.mode == 'Lock Zt' and point_idx < 7:
                        print("  -> ⚠️ 锁定模式：系统接管 h(x) 以维持厚度，无法手动拖拽！")
                        return
                    if self.mode == 'Lock Yc' and 7 <= point_idx < 14:
                        print("  -> ⚠️ 锁定模式：系统接管 alpha(x) 以维持弯度，无法手动拖拽！")
                        return
                    self.active_var_idx = point_idx 
    ...
    def on_motion(self, event):
        ...
        # 数学底层直接覆盖，绝不使用伪逆求点，保证曲线绝对平滑！
        if self.mode in ['Lock Zt', 'Lock Yc']:
            pass # 由 get_bce_airfoil 内部进行绝对刚性几何覆盖

        self.update_plots()

    def update_plots(self):
        try: 
            Y_up, Y_dn, Y_bce_cam = get_bce_airfoil(self.p, X_target, mode=self.mode, Zt_ref=self.Zt_ref, Yc_ref=self.Yc_ref)
        except: return 
        
        self.line_up.set_ydata(Y_up)
        self.line_dn.set_ydata(Y_dn)
        self.line_bce_cam.set_ydata(Y_bce_cam)
